...
PHY_MODE="${PHY_MODE:-qemu}"   # faketrx | virtphy | qemu
...
BRIDGE_SCRIPT="${QEMU_DIR}/l1ctl_bridge.py"
...
if [ "$PHY_MODE" = "virtphy" ]; then
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # MODE QEMU : bridge (built-in TRX) + osmo-bts-trx + QEMU Calypso L1
    #
    #   BTS ←5700/5802→ BRIDGE ←uart→ QEMU
    #                      ↕ L1CTL
    #                    mobile
    # ─────────────────────────────────────────────────────────────────────────
    ...
    for ms in $(seq 1 "$N_MS"); do
        ...
        # Bridge: built-in TRX server on BTS ports (5700) + L1CTL + UART
        bridge_cmd="python3 ${BRIDGE_SCRIPT} ${uart_sock} ${local_l2} ${BTS_PORT_BASE}"
    ...
    echo -e "${GREEN}=== [5b/10] osmo-bts-trx ===${NC}"
...
echo -e "  ${YELLOW}PulseAudio skip (run_tmp)${NC}"
echo ""
