#!/bin/bash
# ══════════════════════════════════════════════════════════════════════════════
# /opt/osmo-launch.sh — Orchestrateur osmo-nitb-for-calypso (lab + web dashboard)
#
# Usage :
...
LAB_DIR="/opt/osmo-nitb-for-calypso"
...
banner() {
    echo -e "${CYAN}${BOLD}"
    echo "╔══════════════════════════════════════════════════════════════╗"
    echo "║  osmo-nitb-for-calypso — GSM/EGPRS Multi-PLMN + Web Dashboard          ║"
    echo "╚══════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
# ══════════════════════════════════════════════════════════════════════════════
start_lab() {
    echo -e "${GREEN}[lab] Démarrage osmo-nitb-for-calypso...${NC}"
    ...
    cd "$LAB_DIR"
    bash ./lancement/start.sh
}

stop_lab() {
    echo -e "${YELLOW}[lab] Arrêt osmo-nitb-for-calypso...${NC}"
    if [ -f "$LAB_DIR/start.sh" ]; then
        cd "$LAB_DIR"
        bash ./lancement/start.sh stop
    fi
    # Nettoyage complet si lancement/start.sh stop ne suffit pas
...
do_auto() {
    ...
    # Lancer lancement/start.sh en mode non-interactif avec des défauts
