#!/bin/bash
# lancement/start.sh — Lance la stack Osmocom GSM multi-opérateurs
#
...
enable_user_loopback() {
    ...
    loopback_script="/home/${target_user}/osmo-nitb-for-calypso/loopback.sh"
...
setup_wan_interop() {
    ...
    if [ ! -f "$script_path" ]; then
        echo -e "${RED}[WAN] reseau/setup-wan-interop.sh introuvable${NC}"
        return 1
    fi
    chmod +x "$script_path"
...
# ${OQC_ROOT}/run.sh dans un gnome-terminal (bloc BRIDGE_QEMU).
start_qemu_poc() {
    echo -e "${GREEN}PoC QEMU Calypso — bring-up minimal + osmo-qemu-calypso/run.sh...${NC}"
    QEMU_POC=1
    start_bridge_mode
}
...
start_bridge_mode() {
    ...
    for i in $(seq 1 "$n_operators"); do
        ...
        # ici le run.sh de osmo-qemu-calypso : il sera lancé dans un gnome-terminal
        ...
        # En qemu : ${OQC_ROOT}/run.sh n'est PLUS lance ici dans un
    ...
    # `docker exec -ti` sur le run.sh calypso. Quand run.sh sort, lancement/start.sh sort.
    ...
    if [ "${BRIDGE_QEMU:-0}" = "1" ]; then
        ...
        exec docker exec -ti "$_qemu_container" bash -c \
            "cd ${NITB_TREE} && NO_MENU=1 MODE='${HANDOFF_MODE}' QEMU_CHOICE='${HANDOFF_QEMU_CHOICE}' ENCRYPTION='${ENCRYPTION}' ${_wan_env} ./start-direct.sh"
    fi
}
...
choose_network_mode() {
    ...
    choice=$(wt_menu "Que lancer ?" "Premier choix :" \
        "qemu"    "PoC QEMU Calypso — osmo-qemu-calypso/run.sh (défaut)" \
...
# Peut précéder le mode :  ./lancement/start.sh quick            → menu, build rapide
#                          ./lancement/start.sh quick virtual    → bridge, build rapide
#                          ./lancement/start.sh normal           → menu, build --no-cache
# Ou via env :             OSMO_QUICK=1 ./lancement/start.sh
case "${1:-}" in
...
#      ./lancement/start.sh qemu|virtual|hw   → mode direct, sans menu
#      ./lancement/start.sh                   → menu whiptail complet (qemu/virtual/hw) [DÉFAUT]
#      OSMO_MULTI=1  ./lancement/start.sh     → multi-opérateurs SS7 (bridge) direct
#      OSMO_MENU=0   ./lancement/start.sh     → PoC QEMU direct (ancien défaut, sans menu)
case "${1:-}" in
    qemu)    NETWORK_MODE="qemu" ;;
    virtual) NETWORK_MODE="bridge" ;;
    hw)      NETWORK_MODE="host" ;;
