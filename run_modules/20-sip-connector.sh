...
# La socket MNCC est-elle RACCORDÉE ? En écoute seule, seul le MSC apparaît ;
# une fois la passerelle connectée, ss montre plusieurs entrées pour ce chemin.
_sipc_mncc_connected() {
    local p; p="$(_sipc_mncc)"
    [ "$(ss -xH 2>/dev/null | grep -c -- "$p")" -ge 2 ]
}
