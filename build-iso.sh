...
echo -e "  ${GREEN}✓${NC} /opt/osmo_egprs"
...
update-initramfs -c -k "$KERNEL" 2>/dev/null || true
...
cat > "$ROOTFS/etc/motd" <<'MOTD'
  ...
  ╠══════════════════════════════════════════════════════════════╣
  ║  cd /opt/osmo_egprs && sudo ./start.sh    ← lancer le lab   ║
  ║  http://localhost:8080                    ← dashboard web    ║
  ║  /opt/osmo_egprs/checks/global_check.sh  ← diagnostics      ║
  ╚══════════════════════════════════════════════════════════════╝
...
grub-mkrescue -o "$OUTPUT" "$ISOROOT" \
    --product-name "osmo_egprs" -- -volid "$LABEL" 2>/dev/null
