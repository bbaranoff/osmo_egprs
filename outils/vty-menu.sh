...
connect_vty() {
    ...
    expect ./outils/vty-connect.exp "$1" "$3" "$4"
        ...
        case $CHOICE in
            ...
            8) 
                ...
                if [[ -n "$GRP_IP" ]]; then
                    ...
                    tput cnorm
                    expect ./outils/vty-connect.exp "$OP" 4247 "bb" "$GRP_IP"
                fi
                ;;
        esac
    done
...
while true; do
    ...
    if [[ "$MAIN_TYPE" == "CORE" ]]; then
        ...
        if [[ -z "$OPS" ]]; then
            whiptail --msgbox "Aucun opérateur détecté ! Vérifiez lancement/start.sh" 10 50
            continue
        fi
