#!/usr/bin/env bash
# matrix.sh <burst|si> — MATRICE DYNAMIQUE du treillis 51-multiframe (cote mobile).
#
# Deux vues "temps/canal" complementaires des 2 FFT (frequence) :
#   matrix.sh burst  -> dynamique des BURSTS (TRXD ts0 DL :5702) :
#                       lignes = FN%51 (structure canal), colonnes = temps,
#                       couleur = vide / dummy(idle) / reel(actif).
#   matrix.sh si     -> dynamique des SI/L2 decodes (GSMTAP :4729/:4730) :
...
#
# Usage : ./matrix.sh burst    |    ./matrix.sh si
#         CONTAINER=osmo-operator-66 TS=0 W=240 ./matrix.sh burst
set -euo pipefail
MODE="${1:?usage: matrix.sh <burst|si>}"
...
SNIFF=${OQC_ROOT}/opt-gsm-scripts/gsm_sniff.py
